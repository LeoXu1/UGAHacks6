...
public class Wheel extends JPanel {
	...
	public void hasBorders(boolean borders) {
		/*
		 * Borders on/off. If switched on, borders of sections and circle + circle
		 * center will be visible.
		 */
		hasBorders = borders;
		_image = null;
		_spinOnOff = false;
	...
	public int getRadius() {
		/*
		 * Get radius of the wheel. The radius is set in DrawImage method based on the
		 * dimensions of this.
		 */
		return _radius;
	}
	...
	public void setListOfStrings(ArrayList<String> list) throws Exception {
		/*
		 * Set list of strings displayed inside the sections of the wheel. The initial
		 * list is set in constructor method and can be changed during runtime.
		 */
		_noElem = list.size();
		if (_noElem > LIMIT)
	...
	public double getSpinSpeed() {
		/*
		 * Get current spinning speed. If the spinning is off, it returns 0.
		 */
	...
	public String getSelectedString() {
		/*
		 * Get current selection. Returns the string which is displayed in the section
		 * of the wheel that is currently positioned between 0 and delta degrees. The
		 * idea is to get the number of deltas in current rotationAngle. This number is
		 * added to the size of the string arraylist, and then MODed by the size of the
		 * string arraylist, in order to avoid negative indices.
		 */
	...
	public Wheel(ArrayList<String> listOfStrings) throws Exception {
		/*
		 * Constructor of the class. Sets the string arraylist, adds mouse listeners and
		 * stast TimerTask to measure the rotation speed.
		 */
...
//				}
		...
		addMouseMotionListener(new MouseAdapter() {
			...
			public void mouseDragged(MouseEvent e) {
				...
				double k1 = (_mouseDragPosition.getY() - _rotationCenter.getY())
						/ (_mouseDragPosition.getX() - _rotationCenter.getX());
	...
	public void paintComponent(Graphics g) {
		/*
		 * Paintcomponent - if the image is null, create it and then draw it whilst
		 * keeping the current rotation. The image can be larger than the displaying
		 * area, so after it is drawn it needs to be placed properly.
		 */
		super.paintComponent(g);

		if (_image == null) {
			_image = drawImage();
			_rotationCenter = new Point2D.Double(this.getWidth() - _image.getWidth(null) + _center.getX(),
					this.getHeight() / 2);
			_imagePosition = new Point2D.Double((int) (this.getWidth() - _image.getWidth(null)),
	...
	private BufferedImage drawImage() {
		/*
		 * Calculate all the necessary parameters for the wheel and draw it section by
		 * section.
		 */
		...
		if (hasBorders) {
			...
			g2d.fillArc((int) _center.getX() - (int) Math.floor(Math.max(0.01 * _radius, 1)),
					(int) _center.getY() - (int) Math.floor(Math.max(0.01 * _radius, 1)),
					(int) Math.floor(Math.max(0.01 * 2 * _radius, 2)),
					(int) Math.floor(Math.max(0.01 * 2 * _radius, 2)), 0, 360);
		}
		...
		for (int i = _noElem - 1; i >= 0; i--) {
			...
			if (hasBorders) {
				g2d.setColor(Color.BLACK);
				g2d.drawLine((int) _center.getX(), (int) _center.getY(), (int) _center.getX() + _radius,
						(int) _center.getY());
			}
			...
			if (_shape == Shape.UMBRELLA)
				...
				fillArc(g2d);
			// Draw string - rotate half delta, then draw then rotate the other half (to
			// have the string in the middle)
	...
	private int calcFontSize(Graphics g, double stringDistanceFromEdge, int maxStringWidth) {
		/*
		 * Calculates the optimal font size for the strings inside the sections. The
		 * strings need to be positioned next to the broader end of the section. The
		 * optimal size will depend on the longest string length and maximum height of
		 * the section in the left border of the rectangle surrounding the string.
		 */
	...
	private void fillArc(Graphics g2d) {
		g2d.fillArc((int) _center.getX() - _radius, (int) _center.getY() - _radius, 2 * _radius, 2 * _radius, 0,
				(int) -Math.ceil(_delta)); // use ceil because of decimal part (would be left empty)
		if (hasBorders) {
			g2d.setColor(Color.black);
			g2d.drawArc((int) _center.getX() - _radius, (int) _center.getY() - _radius, 2 * _radius, 2 * _radius, 0,
					(int) -Math.ceil(_delta));
		}
	}
	...
	private class SpinRunnable implements Runnable {
		/*
		 * Runnable class that handles the spinning of the wheel. It sets the rotation
		 * angle by calculating the speed through time based on deceleration. Each
		 * setRotationAngle call will cause the wheel to be redrawn.
		 */
	...
	public void spinStartAsync(double speed, int direction, double deceleration) throws Exception {
		/*
		 * Method that starts the spinning thread. Parameters: speed => degrees per
		 * second direction => "< 0" = clockwise , "> 0" = counter-clockwise, "=0" =
		 * stand still deceleration => "< 0" = degrees per second per second reducing
		 * speed, "= 0" = perpetual spin, "> 0" = throw exception
		 */
	...
	private class speedTimerTask extends TimerTask {
		/*
		 * TimerTask class that monitors and refreshes the _spinSpeed The speed is
		 * calculated as a difference of two rotation angles over a period of time. We
		 * add the 360 to the "now" angle and then MOD it by 360 to avoid miscalculation
		 * when passing the full circle.
		 */
		@Override
		public void run() {
	...
	private ArrayList<Color> getDefaultColorList() {
		/*
		 * Returns default color list. To be used in case when no explicit color list is
		 * set.
		 */
