...
public class Tick extends JPanel {
	...
	private void adjustPolygon() {
		...
		for (i = 0; i < _polygon.xpoints.length; i++) {
			if (_polygon.xpoints[i] > xmax)
				xmax = _polygon.xpoints[i];
			if (_polygon.xpoints[i] < xmin)
				xmin = _polygon.xpoints[i];
		}
		for (i = 0; i < _polygon.ypoints.length; i++) {
			if (_polygon.ypoints[i] > ymax)
				ymax = _polygon.ypoints[i];
			if (_polygon.ypoints[i] < ymin)
				ymin = _polygon.ypoints[i];
		}
		int width = xmax - xmin;
		// scale polygon
	...
	public void paintComponent(Graphics g) {
		/*
		 * Paintcomponent. If custom polygon is not set, use default triangle.
		 */
